...
SECRET  = 'washqueue-x7k2m'  # ← ganti sesukamu, ini URL rahasia admin
...
def get_antrian():
    ...
    conn.close()
    return [
        {"nomor": r[0], "nama": r[1], "jenis": r[2],
         "email": r[3], "no_hp": r[4], "status": r[5]}
        for r in rows
    ]
...
@app.route(f'/{SECRET}')
def admin():
    return render_template('admin.html', antrian=get_antrian(), secret=SECRET)

@app.route(f'/{SECRET}/panggil/<int:nomor>')
def panggil(nomor):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
@app.route(f'/{SECRET}/selesai/<int:nomor>')
def selesai(nomor):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
# ─── DB VIEWER (juga disembunyikan) ──────────────────────────────

@app.route(f'/{SECRET}/db-view')
def db_view():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
if __name__ == '__main__':
    ...
    print(f"Admin panel: http://127.0.0.1:5000/{SECRET}")

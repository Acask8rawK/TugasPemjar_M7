# WashQueue v3 - Sistem Antrian Cuci Motor dan Mobil
...
Pada versi 3, sistem tetap menggunakan database SQLite untuk data antrian dan log email, serta menambahkan login admin berbasis session Flask agar dashboard admin tidak lagi bergantung pada secret route.
...
| Environment variable | `python-dotenv` | Membaca konfigurasi email dan `FLASK_SECRET_KEY` dari file `.env`. |

## Struktur File Utama

```text
antrianCuci_kel1_v2/
├── app.py                # Aplikasi Flask, route web, login admin, database, dan UDP/email
├── create_admin.py       # Script membuat akun admin lokal di SQLite
...
FLASK_SECRET_KEY=ganti_dengan_secret_key_panjang_dan_acak
```

Gunakan Gmail App Password, bukan password utama akun Google. `FLASK_SECRET_KEY` dipakai Flask untuk menandatangani session login admin.

### 5. Buat akun admin

Jalankan script berikut untuk membuat akun admin pertama di database SQLite:

```bash
python create_admin.py
```

Ikuti prompt username dan password yang muncul di terminal.
...
Route admin dilindungi oleh decorator `admin_required`, sehingga admin harus login sebelum membuka dashboard, memanggil antrian, menyelesaikan antrian, atau melihat database.

### 8. Jalankan UDP listener
...
### 9. Melihat isi database dari terminal
...
### 3. Dashboard admin dengan login

Route login admin adalah `/admin/login`.

Setelah berhasil login, dashboard admin hanya bisa diakses oleh pengguna yang sudah terautentikasi karena route dibungkus decorator `admin_required`.
...
Route yang memproses aksi admin berada di area yang dilindungi login, misalnya endpoint panggil dan selesai di balik `admin_required`.
...
Saat admin menandai antrian selesai dari dashboard yang sudah login, aplikasi:
...
Database viewer berada di area admin dan dilindungi oleh login/session.
...
- `GET` dan `POST /admin/login` untuk login admin,
- route dashboard dan aksi admin yang dilindungi `admin_required` untuk memanggil pelanggan, menyelesaikan antrian, dan melihat database.
...
## Perbedaan Implementasi v2 dan v3

| Aspek | Versi 2 | Versi 3 |
|------|---------|---------|
| Proteksi admin | Secret route berbasis `SECRET` di `app.py`. | Login admin autentikasi session dengan `admin_required`. |
| Akses dashboard | URL admin tersembunyi, tetapi tetap bergantung pada tahu alamatnya. | Admin harus login melalui `/admin/login` sebelum masuk dashboard. |
| Pembuatan admin | Belum ada alur resmi untuk membuat akun admin lokal. | Akun admin dibuat manual lewat `create_admin.py`. |
| Konfigurasi keamanan | `SECRET` dipakai untuk menyamarkan route. | `FLASK_SECRET_KEY` dipakai untuk session Flask. |
| UDP/SMTP | Tetap mengirim broadcast UDP dan email SMTP saat panggil. | Perilaku UDP/SMTP tetap sama, hanya alur akses admin yang diubah. |
| Database | SQLite `washqueue.db` untuk antrian dan log email. | Tetap SQLite `washqueue.db` dengan login admin di atasnya. |
...
Admin login di /admin/login
        |
        v
Admin buka dashboard yang dilindungi admin_required
        |
        v
Admin klik Panggil
        |
...
1. Menambahkan manajemen akun admin yang lebih lengkap, misalnya ubah password atau tambah admin dari UI.
...
WashQueue v3 menerapkan konsep Pemrograman Jaringan melalui kombinasi HTTP/TCP, SMTP/TCP, dan UDP broadcast. TCP dipakai untuk komunikasi yang membutuhkan keandalan, seperti web dan email. UDP dipakai untuk broadcast cepat di jaringan lokal. SQLite membuat data antrian lebih persisten dibanding versi 1 yang masih memakai list di memori, sementara login admin berbasis session membuat dashboard lebih aman dibanding secret route v2.
